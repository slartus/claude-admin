...
class AddProjectUseCase(private val projects: ProjectRepository) {
    suspend operator fun invoke(path: String, name: String? = null, provider: AiProvider = AiProvider.CLAUDE): Result<Project> {
        val dir = File(path)
        ...
        return runCatching { projects.add(dir.absolutePath, name, provider) }
    }
}
